...
function Login() {
  ...
  function handleLogin() {
    ...
    .then((userCredential) => {
        addDoc(collection(db, "users"), { email, pswd });
        const user = userCredential.user.uid
        console.log(userCredential, "usercrandentials");
        console.log(user, "users")
        if( email && pswd !== userCredential.user.uid ) {
  ...
  return (
    ...
    <section className="bg-gray-50 dark:bg-gray-900" >
      <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0" >
        <h1 className="flex items-center mb-10 text-4xl font-semibold text-gray-900 dark:text-white">
          Piks.pixel
        </h1>
        <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6">
              ...
              <p>Create your account? <Link to="/Signup">Signup</Link></p>
            </form>
          </div>
        </div>
      </div>
